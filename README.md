...
1. Web ブラウザで http://ntt.setup（192.168.1.1）を開く  
    * ユーザー名：user
    * パスワード：****
2. [詳細設定]-[静的IPマスカレード]-[エントリ番号] を選択し各種設定  
    * エントリ番号：1
        * 優先順位：1
        * 接続インタフェース名：メインセッション(ISP1)
        * 宛先IPアドレス：192.168.X.X
        * 宛先IPアドレス：自分のWAN側IPアドレス
        * 変換対象プロトコル：TCP
        * 宛先ポート：20 ←FTP
        * 変換対象ポート：21 ←FTP
    * エントリ番号：2
...
実行環境：[NTT PR-S300SE]（22.01）  
[SoftBank E-WMTA2.3](https://torisetsu.biz/products/0000199609/)  
CentOS Stream 8、Windows 11  
作成者：夢寐郎  
作成日：2023年3月XX日  
[[TOP]](#TOP)  
