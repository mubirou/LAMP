...
<a id="202303151240-MAC"></a>

1. Linux の IP、MAC アドレス等を調べる  
    ```
    # ip address
    1: lo: ...
        ...
...
1. ルータの設定画面を開く（172.16.255.254／192.168.X.1） 
    * ユーザー名：user
    * パスワード：****（初期値 user）
...
1. [ルーター機能の設定]-[IPアドレス/DHCPサーバの設定] で以下の通り設定
    * 固定割当：No.1：  
        割当IP：192.168.X.XX  
        [MACアドレス](#202303151240-MAC)：XX:XX:XX:XX:XX:XX  
1. [設定を保存する] を選択、ルーターの再起動
...
1. [ドメイン名とIPアドレスを紐づける](https://www.tagtec.net/onamaecom-dns-setting)（非常に判り難い）  
（**注意：この項目が消えている場合がある**）  
（設定の概要は以下の通り）  
    * ホスト名：**www**
...
1. 同様の設定で「**ホスト名を空白**」にしたものも [追加] 💡  
1. ルーター外からドメイン名で開ければ大成功！

***
👉 **ルーター内からドメイン名でアクセスする方法（Windows版）**  
...
1. 接続の確認（Windowsコマンドプロンプト）
    ...
    192.168.x.xx からの応答: バイト数 =32 時間 <1ms TTL=64
    192.168.x.xx からの応答: バイト数 =32 時間 <1ms TTL=64
    …
    > ping mubirou.com <-pingテスト（〇〇.com）
    www.mubirou.com [192.168.x.xx]に ping を送信しています…
    192.168.x.xx からの応答: バイト数 =32 時間 <1ms TTL=64
    192.168.x.xx からの応答: バイト数 =32 時間 <1ms TTL=64
    …
...
👉 **ルーター内からドメイン名でアクセスする方法（Android版）**  

 

作成者：夢寐郎  
作成日：2023年03月28日  
更新日：2023年10月30日「非常にわかりにくい」を追加  
[[TOP]](#TOP)  
...
1. [準備](#202303262032-A)
1. [電子証明書の作成](#202303262032-D)
1. [ポート開放](#202303262032-E)
1. [https:// アクセス可能化](#202303262032-F)
1. [http:// → https:// 自動接続化](#202303262032-G)
1. [動作確認](#202303262032-H)
1. [電子証明書の自動更新](#202303262032-B)
1. [電子証明書の再発行](#202303262032-C)（オプション）
...
1. [certbot](https://avinton.com/academy/creating-ssl-certificate-by-certbot/) のインストール  
    ```
    # ln -s /var/lib/snapd/snap /snap ←シンボリックシンクを作成
    # ll /snap
...
<a id="202303262032-E"></a>

👉 [ルーターのポート開放](#202303151240)（TCP443番）


<a id="202303262032-F"></a>
